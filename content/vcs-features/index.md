...
在很大程度上，基于进行原子提交的任何版本控制系统（VCS）都可以实现基于主干的开发（剧透：CVS之后都可以）。尽管如此，在工具间生产力和管理都不尽相同。
...
事实证明，有许多生产力相关的原因使得团队放弃了一项技术，进而转向另一种技术。这项技术不管是商业用途还是开源的都无关紧要，至少对最终用户来说这并不是他们关心的。
...
如果你从源代码中检出过一次以上，任何后续的拉取、更新、同步操作会将把最新的代码和上一次检出的时的差异拉下来。如果快速地进行两次更新，则第二次更新可能什么也没有。版本控制工具确认远程主仓库有没有更新所需的时间可以作为衡量其速度的最大依据。
...
那些在签出时只保持最新版本的技术，通常需要遍历整个目录去查找改动的文件并和服务器一一的进行握手。这些技术显然是指Subversion及其前身CVS。如果将这些操作打包起来将会有一定的速度提升，但代码树的广度和深度又会使之变慢。Perforce和服务器端始终保持同步操作因而操作更快，你的源代码树和每个文件的版本都记录在内存中。当然，代价是它需要维护文件的只读位（请确保并使用默认处理Perforce交互的IDE）。Perforce可以有效地、快速地进行这种连续的拉取、更新、同步测试，因为它已经知道问题的答案。
...
CVS、Subversion和Perforce提供了检出子目录的功能。至少说在Monorepo中，如果你在主干中重复地放置了服务和应用程序，你会认为这是一个很好的功能。
...
Subversion和Perforce将修改过的文件的差异发送到服务器。在这个过程中，目录的遍历操作会减缓这个速度。此外还不可避免地会和服务器有一些额外的通讯。
...
Git和Mercurial也是一样的，但是在推送到远程仓库之前，它允许你在本地以非常快的速度提交。在代码被推送到共享的远程仓库之前，这两个工具要求你首先进行拉取操作。
...
开发人员（但愿是结对的）必须熟练掌握如何将本地提交与远端主干的提交进行合并，也必须掌握如何与本地工作副本中正在进行的修改进行合并。你会注意到实行主干开发的团队会更频繁地合并。这些都是较小的合并，它们被隐含地合并到你的工作副本。
...
Perforce的三向合并工具（P4Merge）本身就足够好，这对使用其他VCS技术的团队来说是有吸引力的。但对于其他技术而言，P4Merge只是一个没有实用性的配置选项而已。
...
接下来，GitHub（不是Git）通过内置的代码审查工具（和工作流程）为非Google开发者世界服务。这又是一个有效地预提交工具，或至少在提交给主分支时。
...
在过去和现在都有其他技术的代码进行审查，如Crucible（Atlassian）、UpSource（JetBrains）、Gerrit、Phabricator，但整合到平台的体验是其中的关键。GitLab和RhodeCode是新兴平台。
...
持续集成测试与代码审查功能相辅相成。变更被推送到代码审查分支时，或者最迟在进入代码审查的变更时，CI服务器便开始工作并验证这些变更。这些变更将会经过常规构建，或者更多的其他步骤。代码审阅者都可以查看执行的结果。Facebook提供了服务级别的协议，它规定一个提交放在“需求代码审查”队列之后十分钟内就必须要完成构建工作。
...
在现实生活中，VCS平台只提供钩子（可能的是基于web的钩子）给其他CI服务器。VCS平台和CI服务器最好匹配是对对于任何分支（包括代码审查分支（forks））都可以这么做。
...
企业越大，他们越有可能以文件粒度的方式进行权限划分。例如，对于基于主干开发的团队来说，“可以提交到主干，但不能创建发行分支”将是比较常见的。但并非所有的VCS技术都支持。例如，Git不会，但一些有经验的门户网站增加了这种功能。实际上，你不能阻止人们在DVCS中创建分支，但可以防止他们将其推送到共享仓库。
...
更深层次上，一些企业希望在仓库中的一个分支中的目录级别下设置权限。这可能和一个门一样简单，以确保在提交时发挥作用，也可以在Monorepo实现中保护一些秘密。最后这一点与现代软件工程中的“共同代码所有权”理念相反。
...
许多团队，特别是Monorepo配置或大型二进制文件（如游戏公司）的团队希望为其仓库提供无限的服务器端存储。Git和Mercurial正在努力实现更大的容量，但还有一些工作要做——如何干净利落并且安全地减少客户端克隆历史的大小（同时仍然可以将更改推回到服务器）仍是一个挑战。
...
微软在2071年早些时候发布了Git虚拟文件系统(~~GitVFS~~ GVFS{{< ext url="https://github.com/Microsoft/GVFS" >}})，它技术构建在他们在内部重编译Perforce的项目（SourceDepot - 从1998年至201x使用的）以及其商业性产品Team Foundation Server（TFS）的原生VCS之上。